...
presion_sistolica = st.sidebar.slider(
    "Presión Sistólica",
    80,
    220,
    120
...
presion_diastolica = st.sidebar.slider(
    "Presión Diastólica",
    50,
    150,
    80
...
consume_alcohol = st.sidebar.selectbox(
    "¿Consume Alcohol?",
    ["No", "Sí"]
)

actividad_fisica = st.sidebar.selectbox(
    "Actividad Física",
    ["Baja", "Media", "Alta"]
)

enfermedad_cardiaca = st.sidebar.selectbox(
    "¿Enfermedad Cardíaca?",
    ["No", "Sí"]
)
...
with col2:
    if st.button("🔍 Predecir Colesterol"):
        ...
        st.metric(
            label="Colesterol Estimado",
            value=f"{prediccion:.2f} mg/dL"
        )
...
st.markdown("---")
st.caption(
    "Modelo Predictivo de Colesterol desarrollado con Python, Scikit-Learn y Streamlit."
)
